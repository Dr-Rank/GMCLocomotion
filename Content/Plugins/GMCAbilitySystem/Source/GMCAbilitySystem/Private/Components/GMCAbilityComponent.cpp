...
void UGMC_AbilitySystemComponent::TryActivateAbilitiesByInputTag(const FGameplayTag& InputTag, const UInputAction* InputAction, bool bFromMovementTick)
{
	for (const TSubclassOf<UGMCAbility>& ActivatedAbility : GetGrantedAbilitiesByTag(InputTag))
	{
		...
		if(AbilityCDO && bFromMovementTick == AbilityCDO->bActivateOnMovementTick){
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Trying to Activate Ability: %s from %s (bMovTick=%d, CDO_bMovTick=%d)"), *GetNameSafe(ActivatedAbility), bFromMovementTick ? TEXT("Movement") : TEXT("Ancillary"), bFromMovementTick, AbilityCDO->bActivateOnMovementTick);
			bool bResult = TryActivateAbility(ActivatedAbility, InputAction, InputTag);
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] TryActivateAbility result: %s for %s"), bResult ? TEXT("SUCCESS") : TEXT("FAILED"), *GetNameSafe(ActivatedAbility));
		}
		else if(AbilityCDO) {
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] FILTERED OUT: %s (bFromMovementTick=%d != CDO->bActivateOnMovementTick=%d)"), *GetNameSafe(ActivatedAbility), bFromMovementTick, AbilityCDO->bActivateOnMovementTick);
		}
	}
}
...
bool UGMC_AbilitySystemComponent::TryActivateAbility(const TSubclassOf<UGMCAbility> ActivatedAbility, const UInputAction* InputAction, const FGameplayTag ActivationTag)
{
	...
	if (!AbilityCDO->bAllowMultipleInstances)
	{
		...
		if (GetActiveAbilityCount(ActivatedAbility) > 0) {
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Ability Activation for %s Stopped (Already Instanced)"), *GetNameSafe(ActivatedAbility));
			return false;
		}
	}
	...
	if (!CheckActivationTags(AbilityCDO)){
		UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Ability Activation for %s Stopped By Tags"), *GetNameSafe(ActivatedAbility));
		return false;
	}
...
bool UGMC_AbilitySystemComponent::CheckActivationTags(const UGMCAbility* Ability) const {
	...
	for (const FGameplayTag Tag : Ability->ActivationRequiredTags)
	{
		if (!HasActiveTag(Tag))
		{
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Ability %s can't activate, missing required tag: %s"), *GetNameSafe(Ability), *Tag.ToString());
			return false;
		}
	}
	...
	for (const FGameplayTag Tag : Ability->ActivationBlockedTags)
	{
		if (HasActiveTag(Tag))
		{
			UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Ability %s can't activate, blocked by tag: %s"), *GetNameSafe(Ability), *Tag.ToString());
			return false;
		}
	}
	...
	{
		UE_LOG(LogGMCAbilitySystem, Warning, TEXT("[GMAS_DIAG] Ability %s can't activate, blocked by query: %s"),
			*GetNameSafe(Ability), *Ability->ActivationQuery.GetDescription());
		return false;
	}
